...
def run(train_date: date, val_date: date, out_path: str):
    ...
    try:
        ...
        logger.error(f"Error in training: string{e}")
        raise e



@click.command("Trains the model")
@click.option("--train-date", required=True, help = "Training Month in the format YYY-MM")
@click.option("--val-date", required=True, help = "Validation Month in the format YYY-MM")
@click.option("--model-save-path", required = True, help = "Path where model is saved")
